...
<div class="card w-full mx-auto mb-10 flex-col sm:flex-row items-center justify-between shadow hover:shadow-lg p-4 cursor-pointer" on:click={(event) => openVacancie(event)}>
  <div class="w-full flex flex-col order-2 sm:order-1 pt-2 ">
    ...
    <div class="mt-auto">
      {#each data.skills as skill}
        <button class="btn btn-outline border-accent text-accent btn-xs m-1" id={skill} on:click={() => {dispatch("addFilter", skill);}}>{skill}</button>
      {/each}
    </div>
  </div>
